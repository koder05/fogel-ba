...
namespace RF.WinApp
{
    ...
    public class MyMaskedTextBox : TextBox
    {
        ...
        static MyMaskedTextBox()
        {
            MaskProperty = DependencyProperty.Register("MaskWidth", typeof(string), typeof(MyMaskedTextBox), new UIPropertyMetadata(string.Empty, OnChangeMask));
        }

        private static void OnChangeMask(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var vm = (MyMaskedTextBox)d;
            vm.Mask = (string)e.NewValue;
        }
